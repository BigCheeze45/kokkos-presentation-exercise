 ...
 *
*****************************************************************************/
bool verifyResultVecAdd(float *result, Configuration &configuration) {
 ...
 *
*****************************************************************************/
bool verifyResultJacobi1DCuda(float *result, Configuration &configuration) {
    ...
    for (int i = lower_bound_i; i <= upper_bound_i; ++i) {
        if (j1space(data, upper_bound_T, i) != j1space(result, upper_bound_T, i)) {

            fprintf(stderr, "Position: %d, values: expected %f, found %f\n",
                    i, j1space(data, upper_bound_T, i), j1space(result, upper_bound_T, i));
            success = false;
            break;
        }
    }
 ...
 *
*****************************************************************************/
bool allocateSpace(float **data, Configuration &configuration) {

    // Allocate time-steps 0 and 1
    int requiredSpace = sizeof(float) * (configuration.getInt("Nx") + 2) * 2;
    *data = (float *) malloc(requiredSpace);
    if (*data == NULL) {
        return false;
    }
    return true;
 ...
 *
*****************************************************************************/
void initializeSpace(float *data, Configuration &configuration) {
    ...
    // seed the space.
    for (int i = lower_bound_i; i <= upper_bound_i; ++i) {
        j1space(data, 0, i) = rand() / (float) rand();
    }
    ...
    j1space(data, 0, Nx + 1) = 0;
    j1space(data, 1, Nx + 1) = 0;
}
